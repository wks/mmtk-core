...
pub struct GenCopyPlanFactory<VM: VMBinding>(PhantomData<VM>);

impl <VM: VMBinding> GenCopyPlanFactory<VM> {
    pub fn new() -> Box<GenCopyPlanFactory<VM>> {
        Box::new(GenCopyPlanFactory(PhantomData))
    }
}

impl <VM: VMBinding> PlanFactory<VM> for GenCopyPlanFactory<VM> {
    fn create_plan(
        &self,
        vm_map: &'static VMMap,
